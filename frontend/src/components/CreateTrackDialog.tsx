...
import { Plus, Upload, Music, X, Loader2 } from "lucide-react";
...
export const CreateTrackDialog = () => {
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      // Validate file type
      const validTypes = ['audio/mpeg', 'audio/wav', 'audio/mp3', 'audio/x-wav', 'audio/aiff', 'audio/flac', 'audio/x-aiff'];
      if (!validTypes.includes(file.type) && !file.name.match(/\.(mp3|wav|aif|aiff|flac)$/i)) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (formData.bpm) fd.append("bpm", formData.bpm);
      ...
      toast.success("Track uploaded to VPS successfully");
      queryClient.invalidateQueries({ queryKey: ["tracks"] });
      setOpen(false);
      resetForm();
    } catch (error) {
      console.error("Upload error:", error);
      toast.error(error instanceof Error ? error.message : "Failed to upload track to VPS");
    } finally {
