...
import {
  HardDrive, Folder, FileText, Check, RefreshCw,
  ChevronRight, X, Download, Loader2, Play, ExternalLink, CloudOff,
} from "lucide-react";
import { Button } from "@/components/ui/button";
import { Progress } from "@/components/ui/progress";
import Layout from "@/components/Layout";
import { usePlayer } from "@/components/player/PlayerProvider";
import { api, driveStreamUrl } from "@/lib/api";
...
interface DriveStatus { connected: boolean; connectedAt?: string; tokenExpired?: boolean }
...
export default function GoogleDrive() {
  ...
  const { data: status, isLoading: statusLoading } = useQuery<DriveStatus>({
    ...
    queryFn: () => api.get("/api/drive/status"),
  });

  const { data: filesData, isLoading: filesLoading, refetch } = useQuery<{ files: any[] }>({
    queryKey: ["drive-files", folder],
    queryFn: () => api.get("/api/drive/files", { folderId: folder ?? undefined }),
    enabled: !!status?.connected,
  });

  const connectDrive = async () => {
    const { url } = await api.get<{ url: string }>("/api/drive/auth-url");
  ...
  const importSelected = async () => {
    ...
    for (const fileId of ids) {
      try {
        await api.post("/api/drive/import", { fileId });
        ok++;
      } catch { /* keep going */ }
  ...
  const previewAudio = (file: any) => {
    player.play({
      ...
      artist: "Google Drive",
      fileUrl: driveStreamUrl(file.id),
    });
  };
  ...
  return (
    <Layout>
      <div className="animate-fade-in space-y-6">
        ...
        {status?.tokenExpired && (
          <div className="flex items-center gap-2 p-3 rounded border border-yellow-500/30 bg-yellow-500/8 text-yellow-400 text-sm">
            Token expired — reconnect to restore audio streaming.
          </div>
        )}
        ...
        {filesLoading ? (
          <div className="flex items-center justify-center py-16 gap-2 text-muted-foreground">
            <Loader2 className="w-4 h-4 animate-spin" /><span className="hrl-label">Loading Drive…</span>
          </div>
        ) : (
            ...
            {folders.length > 0 && (
              <section>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-6 gap-2">
                  {folders.map((f) => (
                    <button key={f.id} className="hrl-card p-3 text-center hover:border-yellow-500/30 transition group" onClick={() => enterFolder(f)}>
            ...
            {audioFiles.length > 0 && (
              <section>
                <div className="flex items-center justify-between mb-3">
                  ...
                  <Button
                    variant="ghost" size="sm" className="h-6 text-xs hrl-label"
                    onClick={() => {
                      const allSelected = audioFiles.every((f) => selected.has(f.id));
                      setSelected((s) => {
                        const n = new Set(s);
                        audioFiles.forEach((f) => (allSelected ? n.delete(f.id) : n.add(f.id)));
                        return n;
                      });
                    }}
                  >
                ...
                <div className="hrl-card overflow-hidden">
                  {audioFiles.map((f) => {
                    const isPlaying = player.track?.id === `drive:${f.id}` && player.isPlaying;
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="hrl-label text-muted-foreground">{fmtSize(f.size)} · {f.modifiedTime ? new Date(f.modifiedTime).toLocaleDateString() : "—"}</p>
                        </div>
                        <button
