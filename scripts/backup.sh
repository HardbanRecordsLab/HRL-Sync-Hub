#!/usr/bin/env bash
# HRL Sync — daily backup: Postgres dump + MinIO data volume.
#
#   30 3 * * *  /srv/hrl-sync/scripts/backup.sh >> /var/log/hrl-sync-backup.log 2>&1
#
# Restore:
#   db     : docker compose exec -T db pg_restore -U hrlsync -d hrlsync --clean < hrlsync-<ts>.dump
#   minio  : docker compose stop minio && docker run --rm -v hrl-sync_hrl_minio:/data -v "$PWD":/b \
#            alpine sh -c 'rm -rf /data/* && tar xzf /b/minio-<ts>.tar.gz -C /data' && docker compose start minio
set -euo pipefail
...
echo "$(date -Is) done; kept ${RETENTION_DAYS}d ($(ls -1 "$DEST" | wc -l) files, $(du -sh "$DEST" | cut -f1))"
